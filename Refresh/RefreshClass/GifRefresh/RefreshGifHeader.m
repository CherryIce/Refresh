...
const CGFloat RefreshHeaderHeight = 35.0;
const CGFloat RefreshPointRadius = 5.0;

const CGFloat RefreshPullLen     = 55.0;
const CGFloat RefreshTranslatLen = 5.0;
...
- (instancetype)init {
    if (self = [super initWithFrame:CGRectMake(0, 0, RefreshHeaderHeight, RefreshHeaderHeight)]) {
        [self initLayers];
    }
    return self;
}
...
- (void)initLayers
{
    CGFloat centerLine = RefreshHeaderHeight / 2;
    CGFloat radius = RefreshPointRadius;
    //
    ...
    CGPoint bottomPoint = CGPointMake(centerLine, RefreshHeaderHeight - radius);
    self.BottomPointLayer = [self layerWithPoint:bottomPoint color:bottomPointColor];
    [self.layer addSublayer:self.BottomPointLayer];
    
    CGPoint rightPoint = CGPointMake(RefreshHeaderHeight - radius, centerLine);
    ...
    self.lineLayer.lineWidth = RefreshPointRadius * 2;
...
- (CGPathRef)pointPath {
    return [UIBezierPath bezierPathWithArcCenter:CGPointMake(RefreshPointRadius, RefreshPointRadius) radius:RefreshPointRadius startAngle:0 endAngle:M_PI * 2 clockwise:YES].CGPath;
}
...
- (void)setProgress:(CGFloat)progress
{
    ...
    if (!self.animating) {
        if (progress >= RefreshPullLen) {
            self.y = - (RefreshPullLen - (RefreshPullLen - RefreshHeaderHeight) / 2);
        }else {
            if (progress <= self.h) {
                self.y = - progress;
            }else {
    ...
    //如果到达临界点，则执行刷新动画
    if (progress >= RefreshPullLen && !self.animating && !self.scrollView.dragging) {
        [self startAnimation];
        if (self.handle) {
            self.handle();
        }
...
- (void)setLineLayerStrokeWithProgress:(CGFloat)progress
{
    ...
    if (progress < 0) {
        ...
        //大阶段 0 ~ 3
        NSInteger stage = (progress - (RefreshPullLen - 40)) / 10;
        //大阶段的前半段
        CGFloat subProgress = (progress - (RefreshPullLen - 40)) - (stage * 10);
